...
void CAN_RxMessage(uint8_t FIFONumber, CAN_Rx_Message* RxMessage)
{
    ...
    else
    {
        RxMessage->extenID = (CAN->sRxFIFO[FIFONumber].RXMID_B.STDID << 18) | \
                            CAN->sRxFIFO[FIFONumber].RXMID_B.EXTID;
    }
...
uint8_t CAN_OperatingMode(CAN_OPERATING_MODE_T operatingMode)
{
    ...
    switch ((uint8_t)operatingMode)
    {
        case CAN_OPERATING_MODE_INIT:
            ...
            while((CAN->MSTS_B.SLEEPFLG | (!CAN->MSTS_B.INITFLG)) && (timeout != 0))
            {
                timeout--;
            }
            
            if (CAN->MSTS_B.SLEEPFLG | (!CAN->MSTS_B.INITFLG))
            {
                status = RESET;
            }
            else
        ...
        case CAN_OPERATING_MODE_SLEEP:
            ...
            while((CAN->MSTS_B.INITFLG | (!CAN->MSTS_B.SLEEPFLG)) && (timeout != 0))
            {
                timeout--;
            }
            
            if (CAN->MSTS_B.INITFLG | (!CAN->MSTS_B.SLEEPFLG))
            {
                status = RESET;
            }
            else
