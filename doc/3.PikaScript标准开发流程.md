...
如果要从PikaScript基本类中创建新的类，则需要import PikaObj类包，import PikaObj类包应使用`from PikaObj import *`的引入方式，实际上Pika预编译器是不会编译使用`form`语法导入的包的，这样写只是为了获得python编辑器的只能语法提示，PikaObj是内置于Pika运行时内核的。。
