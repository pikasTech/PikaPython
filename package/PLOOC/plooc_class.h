...
#error You must use __OOC_DEBUG__ (or __OOC_RELEASE__) together with the\
 ...
 *        your code, assuming the variable name of the object pointer is
"ptThis".
 *        If your object pointer has a different name, please feel free to
change
...
#define PLOOC_INVISIBLE(...)                                 \
    uint8_t PLOOC_CONNECT3(chMask_, __LINE__,                \
                           __COUNTER__)[PLOOC_SIZEOF_STRUCT( \
        __VA_ARGS__)] PLOOC_ALIGN(PLOOC_ALIGNOF_STRUCT(__VA_ARGS__));
