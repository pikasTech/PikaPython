...
create_qt_tbl(                     /* 0:OK, !0:Failed */
    ...
    while (ndata) { /* Process all tables in the segment */
        ...
        pb = alloc_pool(
            jd,
            64 * sizeof(int32_t)); /* Allocate a memory block for the table */
        if (!pb)
        ...
        for (i = 0; i < 64; i++) { /* Load the table */
            ...
            pb[zi] = (int32_t)((uint32_t)*data++ *
                               Ipsf[zi]); /* Apply scale factor of Arai
                                             algorithm to the de-quantizers */
        }
    }
...
create_huffman_tbl(          /* 0:OK, !0:Failed */
    ...
    while (ndata) { /* Process all tables in the segment */
        ...
        pb = alloc_pool(
            jd,
            16); /* Allocate a memory block for the bit distribution table */
        if (!pb)
            return JDR_MEM1; /* Err: not enough memory */
        ...
        ph =
            alloc_pool(jd, np * sizeof(uint16_t)); /* Allocate a memory block
                                                      for the code word table */
        if (!ph)
            return JDR_MEM1; /* Err: not enough memory */
        ...
        ndata -= np;
        pd = alloc_pool(jd,
                        np); /* Allocate a memory block for the decoded data */
        if (!pd)
            return JDR_MEM1; /* Err: not enough memory */
        ...
        { /* Create fast huffman decode table */
            ...
            if (cls) {
                tbl_ac = alloc_pool(
                    jd, HUFF_LEN * sizeof(uint16_t)); /* LUT for AC elements */
                if (!tbl_ac)
                    return JDR_MEM1; /* Err: not enough memory */
                jd->hufflut_ac[num] = tbl_ac;
                memset(tbl_ac, 0xFF,
                       HUFF_LEN * sizeof(uint16_t)); /* Default value (0xFFFF:
                                                        may be long code) */
            } else {
                tbl_dc = alloc_pool(
                    jd, HUFF_LEN * sizeof(uint8_t)); /* LUT for AC elements */
                if (!tbl_dc)
                    return JDR_MEM1; /* Err: not enough memory */
                jd->hufflut_dc[num] = tbl_dc;
                memset(tbl_dc, 0xFF,
                       HUFF_LEN * sizeof(uint8_t)); /* Default value (0xFF: may
                                                       be long code) */
            }
            for (i = b = 0; b < HUFF_BIT; b++) { /* Create LUT */
                for (j = pb[b]; j; j--) {
                    ...
                    if (cls) {
                        td = pd[i++] |
                             ((b + 1) << 8); /* b15..b8: code length, b7..b0:
                                                zero run and data length */
                        for (span = 1 << (HUFF_BIT - 1 - b); span;
                             span--, tbl_ac[ti++] = (uint16_t)td)
                            ;
                    } else {
                        td = pd[i++] | ((b + 1) << 4); /* b7..b4: code length,
                                                          b3..b0: data length */
                        for (span = 1 << (HUFF_BIT - 1 - b); span;
    ...
    do {
        if (!bm) {     /* Next byte? */
            if (!dc) { /* No input data is available, re-fill input buffer */
                ...
                if (!dc)
                    return 0 - (int)JDR_INP; /* Err: read error or wrong stream
                                                termination */
            } else {
                dp++; /* Next data ptr */
            }
            ...
            if (flg) {   /* In flag sequence? */
                ...
                if (*dp != 0)
                    return 0 -
                           (int)JDR_FMT1; /* Err: unexpected flag is detected
...
static JRESULT mcu_load(JDEC* jd /* Pointer to the decompressor object */
    ...
    for (blk = 0; blk < nby + 2;
         blk++) { /* Get nby Y blocks and two C blocks */
        cmp = (blk < nby)
                  ? 0
                  : blk - nby + 1; /* Component number 0:Y, 1:Cb, 2:Cr */

        if (cmp && jd->ncomp !=
                       3) { /* Clear C blocks if not exist (monochrome image) */
            ...
            d = huffext(jd, id,
                        0); /* Extract a huffman coded data (bit length) */
            if (d < 0)
            ...
            if (bc) { /* If there is any difference from previous block */
                ...
                jd->dcv[cmp] =
                    (int16_t)d; /* Save current DC value for next block */
            }
            ...
            memset(&tmp[1], 0,
                   63 * sizeof(int32_t)); /* Initialize all AC elements */
            z = 1; /* Top of the AC elements (in zigzag-order) */
            do {
                d = huffext(jd, id, 1); /* Extract a huffman coded value (zero
                                           runs and bit length) */
                if (d == 0)
                    break; /* EOB? */
                if (d < 0)
                    return (JRESULT)(0 -
                                     d); /* Err: invalid code or input error */
                bc = (unsigned int)d;
                ...
                if (bc &= 0x0F) {       /* Bit length? */
                    ...
                    if (!(d & bc))
                        d -= (bc << 1) -
                             1; /* Restore negative value if needed */
                    i = Zig[z]; /* Get raster-order index */
                    tmp[i] =
                        d * dqf[i] >> 8; /* De-quantize, apply scale factor of
                                            Arai algorithm and descale 8 bits */
                }
            } while (++z < 64); /* Next AC element */

            if (JD_FORMAT != 2 || !cmp) { /* C components may not be processed
                                             if in grayscale output */
                if (z == 1 ||
                    (JD_USE_SCALE &&
                     jd->scale == 3)) { /* If no AC element or scale ratio is
                                           1/8, IDCT can be ommited and the
                                           block is filled with DC value */
                    ...
                    block_idct(tmp, bp); /* Apply IDCT and store the block to
                                            the MCU buffer */
                }
            }
        }
...
static JRESULT mcu_output(JDEC* jd, /* Pointer to the decompressor object */
    ...
    if (!JD_USE_SCALE || jd->scale != 3) { /* Not for 1/8 scaling */
        ...
        if (JD_FORMAT !=
            2) { /* RGB output (build an RGB MCU from Y/C component) */
            for (iy = 0; iy < my; iy++) {
                pc = py = jd->mcubuf;
                ...
                for (ix = 0; ix < mx; ix++) {
                    cb =
                        pc[0] - 128; /* Get Cb/Cr component and remove offset */
                    cr = pc[64] - 128;
                    if (mx == 16) { /* Double block width? */
                        if (ix == 8)
                            py += 64 - 8; /* Jump to next block if double block
                                             heigt */
                        pc += ix & 1; /* Step forward chroma pointer every two
                                         pixels */
                    } else {          /* Single block width */
                        pc++; /* Step forward chroma pointer every pixel */
                    }
                    yy = *py++; /* Get Y component */
                    *pix++ = /*R*/ BYTECLIP(yy + ((int)(1.402 * CVACC) * cr) /
                                                     CVACC);
                    *pix++ = /*G*/ BYTECLIP(yy - ((int)(0.344 * CVACC) * cb +
                                                  (int)(0.714 * CVACC) * cr) /
                                                     CVACC);
                    *pix++ = /*B*/ BYTECLIP(yy + ((int)(1.772 * CVACC) * cb) /
                                                     CVACC);
                }
            }
            ...
            for (iy = 0; iy < my; iy++) {
                ...
                for (ix = 0; ix < mx; ix++) {
                    ...
                    *pix++ = (uint8_t)*py++; /* Get and store a Y value as
                                                grayscale */
                }
            }
        }
        ...
        if (JD_USE_SCALE && jd->scale) {
            ...
            for (iy = 0; iy < my; iy += w) {
                for (ix = 0; ix < mx; ix += w) {
                    ...
                    r = g = b = 0;
                    for (y = 0; y < w;
                         y++) { /* Accumulate RGB value in the square */
                        for (x = 0; x < w; x++) {
                            r += *pix++; /* Accumulate R or Y (monochrome
                                            output) */
                    ...
                    *op++ =
                        (uint8_t)(r >> s); /* Put R or Y (monochrome output) */
                    if (JD_FORMAT != 2) {  /* RGB output? */
        ...
        for (iy = 0; iy < my; iy += 8) {
            ...
            for (ix = 0; ix < mx; ix += 8) {
                ...
                if (JD_FORMAT != 2) {
                    *pix++ = /*R*/ BYTECLIP(
                        yy + ((int)(1.402 * CVACC) * cr / CVACC));
                    *pix++ = /*G*/ BYTECLIP(yy - ((int)(0.344 * CVACC) * cb +
                                                  (int)(0.714 * CVACC) * cr) /
                                                     CVACC);
                    *pix++ = /*B*/ BYTECLIP(
                        yy + ((int)(1.772 * CVACC) * cb / CVACC));
                } else {
                    *pix++ = yy;
                }
            }
    ...
    if (rx < mx) { /* Is the MCU spans rigit edge? */
        ...
        for (y = 0; y < ry; y++) {
            ...
            s += (mx - rx) *
                 (JD_FORMAT != 2 ? 3 : 1); /* Skip truncated pixels */
        }
    }
...
JRESULT jd_prepare(
    ...
    jd->inbuf = seg =
        alloc_pool(jd, JD_SZBUF); /* Allocate stream input buffer */
    if (!seg)
        return JDR_MEM1;
    ...
    for (;;) { /* Parse JPEG segments */
        ...
        switch (marker & 0xFF) {
            case 0xC0: /* SOF0 (baseline JPEG) */
                ...
                jd->width =
                    LDB_WORD(&seg[3]); /* Image width in unit of pixel */
                jd->height =
                    LDB_WORD(&seg[1]); /* Image height in unit of pixel */
                ...
                for (i = 0; i < jd->ncomp; i++) {
                    ...
                    if (i == 0) {       /* Y component */
                        if (b != 0x11 && b != 0x22 &&
                            ...
                            return JDR_FMT3; /* Err: Supports only 4:4:4, 4:2:0
                                                or 4:2:2 */
                        }
                        jd->msx = b >> 4;
                        ...
                        if (b != 0x11)
                            return JDR_FMT3; /* Err: Sampling factor of Cb/Cr
                                                must be 1 */
                    }
                    jd->qtid[i] = seg[8 + 3 * i]; /* Get dequantizer table ID
                                                     for this component */
                    if (jd->qtid[i] > 3)
            ...
            case 0xC4: /* DHT - Define Huffman Tables */
                ...
                rc = create_huffman_tbl(jd, seg,
                                        len); /* Create huffman tables */
                if (rc)
                    return rc;
                break;
            ...
            case 0xDB: /* DQT - Define Quaitizer Tables */
                ...
                rc = create_qt_tbl(jd, seg,
                                   len); /* Create de-quantizer tables */
                if (rc)
                    return rc;
                break;
            ...
            case 0xDA: /* SOS - Start of Scan */
                ...
                /* Check if all tables corresponding to each components have
                 * been loaded */
                for (i = 0; i < jd->ncomp; i++) {
                    b = seg[2 + 2 * i]; /* Get huffman table ID */
                    if (b != 0x00 && b != 0x11)
                        return JDR_FMT3; /* Err: Different table number for
                                            DC/AC element */
                    n = i ? 1 : 0;       /* Component class */
                    if (!jd->huffbits[n][0] ||
                        !jd->huffbits[n][1]) { /* Check huffman table for this
                                                  component */
                        return JDR_FMT1;       /* Err: Nnot loaded */
                    }
                    if (!jd->qttbl[jd->qtid[i]]) { /* Check dequantizer table
                                                      for this component */
                ...
                len = n * 64 * 2 +
                      64; /* Allocate buffer for IDCT and RGB output */
                if (len < 256)
                    len = 256; /* but at least 256 byte is required for IDCT */
                jd->workbuf = alloc_pool(
                    jd, len); /* and it may occupy a part of following MCU
                                 working buffer for RGB output */
                if (!jd->workbuf)
                    return JDR_MEM1; /* Err: not enough memory */
                jd->mcubuf = alloc_pool(
                    jd, (n + 2) * 64 *
                            sizeof(jd_yuv_t)); /* Allocate MCU working buffer */
                if (!jd->mcubuf)
                    return JDR_MEM1; /* Err: not enough memory */
                ...
                if (ofs %= JD_SZBUF) {
                    jd->dctr =
                        jd->infunc(jd, seg + ofs, (size_t)(JD_SZBUF - ofs));
                }
                jd->dptr = seg + ofs - (JD_FASTDECODE ? 0 : 1);

                return JDR_OK; /* Initialization succeeded. Ready to decompress
                                  the JPEG image. */
            ...
            case 0xCF:           /* SOF15 */
            case 0xD9:           /* EOI */
                return JDR_FMT3; /* Unsuppoted JPEG standard (may be progressive
                                    JPEG) */

            default: /* Unknown segment (comment, exif or etc..) */
                /* Skip segment data (null pointer specifies to remove data from
                 * the stream) */
