...
#endif /* #ifndef PIKA_HAL */

#ifdef PIKA_HAL
volatile static int pin_inited = 0;
static int _rt_hw_pin_init(void)
#else
int rt_hw_pin_init(void)
#endif /* PIKA_HAL */
{
...
#ifdef RT_USING_PIN
    data->pin_num = _stm32_pin_get(name);
#endif
...
int pika_hal_platform_GPIO_read(pika_dev* dev, void* buf, size_t count) {
    ...
    pika_debug("\r\n=%s==%s=%d=gpio:%d==\r\n", __FILE__, __FUNCTION__, __LINE__,
               data->pin_num);
#ifdef RT_USING_PIN
    level = stm32_pin_read(NULL, data->pin_num);
#endif
    if (level != 1 && level != 0) {
        return -1;
    }
...
#ifdef RT_USING_PIN
    if (level == 0) {
        stm32_pin_write(NULL, data->pin_num, PIN_LOW);
        return 0;
    }
    if (level == 1) {
        stm32_pin_write(NULL, data->pin_num, PIN_HIGH);
        return 0;
    }
#endif
    return 0;
...
#ifdef RT_USING_PIN
    stm32_pin_mode(NULL, data->pin_num, pinMode);
#endif
    return 0;
}
