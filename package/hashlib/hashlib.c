...
Arg* hashlib_md5(PikaObj *self, Arg *data)
{
    ...
    return arg_newObj(self);
}

Arg* hashlib_sha1(PikaObj *self, Arg *data)
{
    size_t size = arg_getBytesSize(data);
    uint8_t *data_h = arg_getBytes(data);
...
Arg* hashlib_sha224(PikaObj *self, Arg *data)
{
    size_t size = arg_getBytesSize(data);
    uint8_t *data_h = arg_getBytes(data);
...
Arg* hashlib_sha256(PikaObj *self, Arg *data)
{
    size_t size = arg_getBytesSize(data);
    uint8_t *data_h = arg_getBytes(data);
...
Arg* hashlib_sha384(PikaObj *self, Arg *data)
{
    size_t size = arg_getBytesSize(data);
    uint8_t *data_h = arg_getBytes(data);
...
Arg* hashlib_sha512(PikaObj *self, Arg *data)
{
    size_t size = arg_getBytesSize(data);
    uint8_t *data_h = arg_getBytes(data);
...
Arg* hashlib_new(PikaObj *self, char *mode)
{
...
void hashlib_update(PikaObj *self, Arg *data)
{
    size_t size = arg_getBytesSize(data);
    uint8_t *data_h = arg_getBytes(data);
    ...
    if (size != 0)
    {
        ...
        case PIKA_HASHLIB_SHA1:
            mbedtls_sha1_update((mbedtls_sha1_context *)context, data_h, size);
            break;
        case PIKA_HASHLIB_SHA224:
        case PIKA_HASHLIB_SHA256:
            mbedtls_sha256_update((mbedtls_sha256_context *)context, data_h, size);
            break;
        case PIKA_HASHLIB_SHA384:
        case PIKA_HASHLIB_SHA512:
            mbedtls_sha512_update((mbedtls_sha512_context *)context, data_h, size);
            break;
        default:
            obj_setErrorCode(self, -1); // not support mode
            break;
...
Arg *hashlib_digest(PikaObj *self)
{
...
char *hashlib_hexdigest(PikaObj *self)
{
...
void hashlib_copy(PikaObj *self)
{
}
