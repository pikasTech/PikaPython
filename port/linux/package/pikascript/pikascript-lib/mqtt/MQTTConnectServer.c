...
int MQTTDeserialize_connect(MQTTPacket_connectData* data,
    ...
    /* If we don't recognize the protocol version, we don't parse the connect
     * packet on the basis that we don't know what the format will be.
     */
    if (MQTTPacket_checkVersion(&Protocol, version)) {
        ...
        if (flags.bits.username) {
            if (enddata - curdata < 3 ||
                ...
                goto exit; /* username flag set, but no username supplied -
                              invalid */
            if (flags.bits.password &&
                (enddata - curdata < 3 ||
                 !readMQTTLenString(&data->password, &curdata, enddata)))
                goto exit; /* password flag set, but no password supplied -
                              invalid */
