...
int _mqtt__MQTT_subscribe(PikaObj* self, char* topic, int qos, Arg* cb) {
    ...
    if (ret == 0) {
        ...
        if (cb != NULL) {
            ...
            sprintf(hash_str, "C%d", hash_time33(topic));
...
void Subscribe_Handler(void* client, message_data_t* msg) {
    ...
    if (strlen(msg->topic_name) <= MQTT_TOPIC_LEN_MAX)
        ...
        memcpy((topic_str + strlen("eventCallBack_")), msg->topic_name,
               MQTT_TOPIC_LEN_MAX);
    }

    pks_eventListener_sendSignal(g_mqtt_event_listener, hash_time33(topic_str),
