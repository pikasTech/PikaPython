...
static int webclient_connect(struct webclient_session *session, const char *URI)
{
...

    {
        ...
        if (session->is_tls)
        {
            socket_handle = __platform_socket(res->ai_family, SOCK_STREAM, PROTOCOL_TLS);
        }
        else
        {
            socket_handle = __platform_socket(res->ai_family, SOCK_STREAM, IPPROTO_TCP);
        }
#else
        socket_handle = __platform_socket(res->ai_family, SOCK_STREAM, IPPROTO_TCP);
#endif
        ...
        if (socket_handle < 0)
        {
            LOG_E("connect failed, create __platform_socket(%d) error.", socket_handle);
        ...
        if (connect(socket_handle, res->ai_addr, res->ai_addrlen) != 0)
        {
            ...
            LOG_E("connect failed, connect __platform_socket(%d) error.", socket_handle);
