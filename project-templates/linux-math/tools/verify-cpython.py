...
def main():
    ...
    if reference.returncode != 0 or reference.stderr:
        print("CPython reference failed: %r" % reference.stderr, file=sys.stderr)
        return 1
    if actual.returncode != 0 or actual.stderr:
        print(
            "PikaPython execution failed: rc=%d stdout=%r stderr=%r"
            % (actual.returncode, actual.stdout, actual.stderr),
            file=sys.stderr,
        )
        return 1
    if actual.stdout != reference.stdout:
        print("CPython: %r" % reference.stdout, file=sys.stderr)
        print("PikaPython: %r" % actual.stdout, file=sys.stderr)
        return 1
